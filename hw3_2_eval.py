# -*- coding: utf-8 -*-
"""hw3_2_eval.ipynb 的副本

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18mrCHcyZ56-zMiX6_lJrT7OatBs9BMx_
"""

# !gdown 1dNO38qQmd44frUxBB9QfQwhokhLz6PP9 -O hw3_data.zip
# !unzip -q ./hw3_data.zip

# !gdown 1mFSflZ22Rw_p2GgRxcX0PJ0lSGWOkflN
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        decoder_layer = DecoderLayer(d_model=d_model,
                                     ...
                                     dropout=dropout)
        self.encoder = timm.create_model('swinv2_base_window8_256', pretrained = False)
...
def eval_score(json_name):
    ...
    print(f"CIDEr: {cider_score} | CLIPScore: {clip_score.to('cpu').item()}")
...
for i in range(10, 22):
    model.load_state_dict(torch.load(f'./vit_swinv2_{i+1}.ckpt', map_location=device))
    for batch_idx, (imgs, cptns_all, file_names) in enumerate(val_loader):
        ...
        if not beam:
            ...
            start_1 = torch.full(size=(imgs.size(0), 1),
                            fill_value=2,
                            dtype=torch.long,
                            device=device)
            start_2 = torch.full(size=(imgs.size(0), 1),
                            fill_value=2,
                            dtype=torch.long,
                            device=device)
            with torch.no_grad():
                logits, attns = model(imgs, start_1)
                next_word = torch.topk(logits, 2, dim=2)
                next_word_id_1 = torch.as_tensor(next_word[1][:,-1,:][:,0].unsqueeze(1)).to(device)
                next_word_id_2 = torch.as_tensor(next_word[1][:,-1,:][:,1].unsqueeze(1)).to(device)
                next_word_p1 = next_word[0][:,-1,:][:,0].to(device)
                next_word_p2 = next_word[0][:,-1,:][:,1].to(device)
                start_1 = torch.cat(
                        (start_1, next_word_id_1), dim=1)
                start_2 = torch.cat(
                        (start_2, next_word_id_2), dim=1)
                # while start.size(1) <= (max_len - 2) and start.nelement():
                while start_1.size(1) <= (max_len - 2):
                    logits_1, attns_1 = model(imgs, start_1)
                    logits_2, attns_2 = model(imgs, start_2)
                    # print(logits.size())
                    next_word_1 = torch.topk(logits_1, 2, dim=2)
                    next_word_2 = torch.topk(logits_2, 2, dim=2)
                    mult_1 = next_word_p1*next_word_1[0][:,-1,:][:,0].to(device)
                    mult_2 = next_word_p2*next_word_2[0][:,-1,:][:,0].to(device)
                    for one_b in range(next_word_p1.size(0)):
                        if (mult_1[one_b].item()>=mult_2[one_b].item()):
                            start_2[one_b, -1] = start_1[one_b, -1]
                            next_word_id_1 = torch.as_tensor(next_word_1[1][:,-1,:][:,0].unsqueeze(1)).to(device)
                            next_word_id_2 = torch.as_tensor(next_word_1[1][:,-1,:][:,1].unsqueeze(1)).to(device)
                            next_word_p1 = next_word_1[0][:,-1,:][:,0].to(device)
                            next_word_p2 = next_word_1[0][:,-1,:][:,1].to(device)
                        else:
                            start_1[one_b, -1] = start_2[one_b, -1]
                            next_word_id_1 = torch.as_tensor(next_word_2[1][:,-1,:][:,0].unsqueeze(1)).to(device)
                            next_word_id_2 = torch.as_tensor(next_word_2[1][:,-1,:][:,1].unsqueeze(1)).to(device)
                            next_word_p1 = next_word_2[0][:,-1,:][:,0].to(device)
                            next_word_p2 = next_word_2[0][:,-1,:][:,1].to(device)
                    # print('n: ', next_word_id)
                    jump = True
                    for wo in range(next_word_id_1.size(0)):
                        if next_word_id_1[wo][0] != 0 or next_word_id_2[wo][0] != 0:
                            jump = False
                            break
                    if jump:
                        break
                    ...
                    start_2 = torch.cat(
                        (start_2, next_word_id_2), dim=1)
                start_1 = start_1.cpu().tolist()
                
                for word in range(len(start_1)):
                    word_string = tokenizer.decode(start_1[word], skip_special_tokens= True)
                    name = os.path.splitext(file_names[word])[0]
                    dictionary[name] = word_string
        # progress2.update(1)
    with open(f'sample_v2_eval_{i+1}.json', "w") as outfile:
        json.dump(dictionary, outfile)
    eval_score(f'sample_v2_eval_{i+1}.json')