...
transforms_1 = T.Compose([
    T.Resize((256, 256)),
    T.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0),
    T.ToTensor(),
...
hw3_2_train = hw3_2_dataset(path_to_datafile, train_json, transforms_1, 'images/train')
hw3_2_val = hw3_2_dataset(path_to_datafile, val_json, transforms_2, 'images/val')
BATCH_SIZE = 4
...
class CLIPScore:
    ...
    def getCLIPScore(self, image, caption):
        ...
        return w * similarity
clip_class = CLIPScore()
cider_class = CIDERScore()
def eval_score(json_name):
    ...
    # CIDErScore
    cider_score = cider_class(predictions, gts)

    # CLIPScore
    clip_score = clip_class(predictions, 'hw3_data/p2_data/images/val/')
...
n_epoch = 18
model.load_state_dict(torch.load(f'./vit_swinv2_{n_epoch}.ckpt'))
...
for epoch in range(EPOCH):
    ...
    for batch_idx, (imgs, cptns_all) in enumerate(train_loader):
        ...
        # cptns = cptns_all.squeeze(0).permute(1,0).to(device, torch.int32)
        ...
        # progress.update(1)
    torch.save(model.state_dict(), f'vit_swinv2_{epoch+1+n_epoch}.ckpt')
    ...
    if epoch>2:
        ...
        with open(f'sample_swinv2_{epoch+1+n_epoch}.json', "w") as outfile:
            json.dump(dictionary, outfile)
        eval_score(f'sample_swinv2_{epoch+1+n_epoch}.json')