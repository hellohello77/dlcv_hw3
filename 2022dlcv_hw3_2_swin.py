...
n_epoch = 30
model.load_state_dict(torch.load(f'./vit_swin{n_epoch}.ckpt'))
...
for epoch in range(EPOCH):
    ...
    for batch_idx, (imgs, cptns_all) in enumerate(train_loader):
        ...
        # progress.update(1)
    torch.save(model.state_dict(), f'vit_swin{epoch+1+n_epoch}.ckpt')
    ...
    #   progress2 = tqdm(total = math.ceil(len(val_loader)))
      ...
      with open(f'sample_swin{epoch+1+n_epoch}.json', "w") as outfile:
          json.dump(dictionary, outfile)
      eval_score(f'sample_swin{epoch+1+n_epoch}.json')