...
class hw3_2_dataset:
    ...
    def __getitem__(self, idx):
        ...
        # print(y.size())
        # pad = torch.zeros((60 - y.size(0), 5), dtype = torch.long)
...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            trans_img = self.preprocess(image).to(self.device).unsqueeze(0)
            image.close()
            total_score += self.getCLIPScore(trans_img, pred_caption)
    ...
    def getCLIPScore(self, image, caption):
        ...
        caption_e = self.model.encode_text(clip.tokenize(caption).to(self.device))
        return 2.5*nn.CosineSimilarity(dim = -1, eps = 1e-8)(image_e, caption_e)


def eval_score():
    # Read data
    predictions = readJSON('sample.json')
...
# os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
model = Transformer(cp_num, 1000, 1024, 6, 4, max_len, 0.1, 0)
model.load_state_dict(torch.load(f'./vit_b_64_pre.ckpt'))
...
for epoch in range(EPOCH):
    ...
    for batch_idx, (imgs, cptns_all) in enumerate(train_loader):
        ...
        # progress.update(1)
    torch.save(model.state_dict(), f'./vit_b_64_{epoch}.ckpt')
