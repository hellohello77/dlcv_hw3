...
def eval_score(json_name):
    ...
    print(set(predictions.keys()) - set(gts.keys()))
    print(set(gts.keys()) - set(predictions.keys()))
...
n_epoch = 22
model.load_state_dict(torch.load(f'./vit_swin_large_{n_epoch}.ckpt', map_location=device))
...
for epoch in range(EPOCH):
    ...
    for batch_idx, (imgs, cptns_all) in enumerate(train_loader):
        ...
        # progress.update(1)
    torch.save(model.state_dict(), f'vit_swin_large_{epoch+1+n_epoch}.ckpt')
...
#   progress2 = tqdm(total = math.ceil(len(val_loader)))
    ...
    with open(f'sample_swin_large_{epoch+1+n_epoch}.json', "w") as outfile:
        json.dump(dictionary, outfile)
    eval_score(f'sample_swin_large_{epoch+1+n_epoch}.json')